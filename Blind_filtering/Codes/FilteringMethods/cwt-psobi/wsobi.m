...
%         wi - intrested segment of data, in the format of [start; end].
% Outputs:
%   H - Matrix of size [m,n], an estimate of the *mixing* matrix. Its
...
% Note 1:     Adapted by Arnaud Delorme and Scott Makeig to process data epochs by
%           computing covariances while respecting epoch boundaries.
% Note 2: Adapted by Davood Fattahi to get time-lag instead of number of
%       correlation matrices. 08/08/2020.
% Note 3: Adapted by Davood Fattahi to calculate the sobi on an specific
%       window.  08/08/2020. 
...
for u=1:m:pm
    ...
    for t = 1:ntrials
        if t == 1
            Rxp=X(:,(wi(1):wi(2))+tau(k-1),t)*X(:,(wi(1):wi(2)),t)'/(wi(2)-wi(1)+1)/ntrials;
        else
            Rxp=Rxp+X(:,(wi(1):wi(2))+tau(k-1),t)*X(:,(wi(1):wi(2)),t)'/(wi(2)-wi(1)+1)/ntrials;
        end
    end
...
%
if nargout>1
    S=V'*X(:,(wi(1):wi(2))); % estimated source activities
end
