...
ConfigSheet=cell(size(files,1),7);
ConfigSheet(1,:)={'file name', 'lower frequency', 'upper frequency', 'voice per octave', 'wavelet name', 'heart components', 'lung components'};
...
%%
%%% cwtpca       
    [S, Fltrs, frange, Us, U, p, Y, WCoefs, FrFltBnk] =cwtpca(data, fs, vpo, wname, fl, fh, {ph,pl});
...
%     set(gca,'xticklabel',(0:.5: 2.5),'FontSize',12)
 ...
 %% write the results on hard disk   
    ...
    waitfor(Results);    
    if Save
    audiowrite(['TempResults\' name{i} '_PCAonCWT_Heart_' num2str(ph) '.wav'],H,fs)
    audiowrite(['TempResults\' name{i} '_PCAonCWT_Lung_' num2str(pl) '.wav'],L,fs)
    
%%
    ConfigSheet(i+1,:)={files(i).name ,fl, fh, vpo, wname, num2str(ph), num2str(pl)};
    end
    end
end
