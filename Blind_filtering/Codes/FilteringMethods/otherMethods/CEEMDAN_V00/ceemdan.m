...
for i=1:NR
    modes_white_noise{i}=emd_alt(white_noise{i});%calculates the modes of white gaussian noise
end

for i=1:NR %calculates the first mode
    temp=x+Nstd*white_noise{i};
    [temp, ~, it]=emd_alt(temp,'MAXMODES',1,'MAXITERATIONS',MaxIter);
    temp=temp(1,:);
    aux=aux+temp/NR;
    iter(i,1)=it;
end
...
while  nnz(diff(sign(diff(x-acum))))>2 %calculates the rest of the modes
    for i=1:NR
        ...
        if tamanio(1)>=k+1
            ...
            noise=Nstd*noise;
            try
                [temp, ~, it]=emd_alt(x-acum+std(x-acum)*noise,'MAXMODES',1,'MAXITERATIONS',MaxIter);
                temp=temp(1,:);
            catch
                it=0;
                temp=x-acum;
            ...
            [temp, ~, it]=emd_alt(x-acum,'MAXMODES',1,'MAXITERATIONS',MaxIter);
            temp=temp(1,:);
        end
        aux=aux+temp/NR;
    iter(i,k+1)=it;    
