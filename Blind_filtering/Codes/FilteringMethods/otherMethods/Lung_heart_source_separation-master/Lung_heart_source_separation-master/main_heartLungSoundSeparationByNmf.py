# -*- coding: utf-8 -*-
"""
Created on Tue Dec 14 12:21:49 2021

@author: Davood Fattahi, An eddition of the other works
"""
...
def find_and_open_audio(filepath):
...
if __name__ == '__main__':
    ...
    db_folder = '..\..\..\..\..\ChestRecords\CurrentDataset'
    filenames = [i for i in os.listdir(db_folder) if i.endswith('.wav')]

    for name, name in enumerate(filenames):    
        try:
            samplerate, audio = wavfile.read(f'{db_folder}/{name}')
        except:
            audio, samplerate = sf.read(f'{db_folder}/{name}')
