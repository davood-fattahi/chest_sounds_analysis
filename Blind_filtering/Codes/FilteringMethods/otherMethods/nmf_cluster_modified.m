function xhat= nmf_cluster_modified(xm, fs, K, reconstruction, TF, options_tf, options_nmf)
%% Paper Information
...
switch TF
    ...
    case "Cochleagram"
        ...
        gf = gammatoneFast(xm, options_tf.num_bin, options_tf.fRange, fs);
        Xm = cochleagram(gf,options_tf.cgL);
        Vm=abs(Xm);
        F = size(Vm,1);
        T = size(Vm,2);
    case "Q-transform"
...
options_nmf.w_update_ind = [];
options_nmf.init_w = Wnew;
options_nmf = rmfield(options_nmf,'r');
[W, H]   = sparse_nmf(Vm,options_nmf);
