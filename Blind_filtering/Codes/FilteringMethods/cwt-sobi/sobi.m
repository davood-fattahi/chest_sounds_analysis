...
for u=1:m:pm
    ...
    for t = 1:ntrials
        if t == 1
            Rxp=X(:,tau(k-1):N,t)*X(:,1:N-tau(k-1)+1,t)'/(N-tau(k-1)+1)/ntrials;
        else
            Rxp=Rxp+X(:,tau(k-1):N,t)*X(:,1:N-tau(k-1)+1,t)'/(N-tau(k-1)+1)/ntrials;
        end
    end
