...
ConfigSheet=cell(size(files,1),8);
ConfigSheet(1,:)={'file name', 'lower frequency', 'upper frequency', 'voice per octave', 'wavelet name', 'tau', 'heart components', 'lung components'};
...
%%% cwtsobi
    ...
    [S, Fltrs, frange, Us, U, p, Y, WCoefs, FrFltBnk] =cwtsobi(data, fs, vpo, wname, fl, fh, tau, {ph, pl});
    
%% plots
    psdY=normalize(pwelch(real(Y'),size(Y,2)./(.05*fs)),'range');
...
%     set(gca,'xticklabel',(0:.5: 2.5),'FontSize',12)
 ...
 %% write the results on hard disk 
    ...
    waitfor(Results);    
    if Save
    audiowrite(['TempResults\' name{i} '_CWT_SOBI_Heart_' num2str(ph) '.wav'],normalize(H)./6,fs)
    audiowrite(['TempResults\' name{i} '_CWT_SOBI_Lung_' num2str(pl) '.wav'],normalize(L)./15,fs)
%%
    ConfigSheet(i+1,:)={files(i).name ,fl, fh, vpo, wname, taustr, num2str(ph), num2str(pl)};
    end
    end
end
