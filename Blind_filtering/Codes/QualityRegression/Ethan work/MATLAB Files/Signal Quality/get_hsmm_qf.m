...
%% OUTPUTS:
% homo- hsmmqf for homomorphic envelope
% hil- hsmmqf for hilbert envelope
% psd- hsmmqf for power spectral density envelope
% wav- hsmmqf for wavelet envelope
...
% [homomorphic_envelope,hilbert_envelope,psd,wavelet_feature] = get_envelopes(audio_data, Fs);
%   
% t_env=homomorphic_envelope;
% homo_S1=abs(mean(t_env(locS1))/mean(t_env(locSysDia)));
% homo_S2=abs(mean(t_env(locS2))/mean(t_env(locSysDia)));
% 
% t_env=hilbert_envelope;
% hil_S1=abs(mean(t_env(locS1))/mean(t_env(locSysDia)));
% hil_S2=abs(mean(t_env(locS2))/mean(t_env(locSysDia)));
% 
% t_env=psd;
% psd_S1=abs(mean(t_env(locS1))/mean(t_env(locSysDia)));
% psd_S2=abs(mean(t_env(locS2))/mean(t_env(locSysDia)));
% 
% t_env=wavelet_feature;
% wav_S1=abs(mean(t_env(locS1))/mean(t_env(locSysDia)));
% wav_S2=abs(mean(t_env(locS2))/mean(t_env(locSysDia)));
